...
make_image() {
    cd ${make_path}
        ...
        out_release=$(echo ${armbian_image_name} | awk -F "bullseye" '{print $1}' | grep -oE '[1-9]{1,3}.[0-9]{1,3}.[0-9]{1,3}' 2>/dev/null)
        out_version=$(echo ${armbian_image_name} | awk -F "bullseye" '{print $NF}' | grep -oE '[1-9].[0-9]{1,3}.[0-9]{1,3}' 2>/dev/null)
        ...
        build_image_file="${tmp_outpath}/Armbian_${out_release}_Aml_${build_soc}_bullseye_${make_version}_$(date +"%Y.%m.%d.%H%M").img"
